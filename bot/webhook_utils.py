...
# ИСПРАВЛЕНИЕ #5: добавляем глобальную переменную для allowed_updates
_allowed_updates_cache = None

def set_allowed_updates(updates: list):
    """Сохраняет список разрешённых типов обновлений для использования в вебхуке."""
    global _allowed_updates_cache
    _allowed_updates_cache = updates


async def check_and_set_webhook(bot: Bot = None):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.telegram.org/bot{config.TOKEN}/getWebhookInfo") as resp:
                ...
                if current_url != expected_url:
                    ...
                    # Используем кэшированный список разрешённых обновлений
                    await bot.set_webhook(url=expected_url, allowed_updates=_allowed_updates_cache)
