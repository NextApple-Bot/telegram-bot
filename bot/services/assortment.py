...
class AssortmentService:
    ...
    async def load_inventory() -> List[Dict[str, Any]]:
        """Загружает весь ассортимент с группировкой по категориям, отсортированный по sort_order (NULLs last)."""
            ...
            async with get_async_session_factory()() as session:
                for _, row in df.iterrows():
                    ...
                    if not category:
                        ...
                        category = Category(name=category_name, sort_order=max_order + 10)
    ...
    async def import_arrival_from_txt(file_path: str) -> Dict[str, Any]:
        ...
        Порядок категорий сохраняется по порядку их первого появления в файле.
        """
        try:
            added_categories = 0
            added_items = 0
            ...
            # Читаем файл и собираем категории по порядку первого появления
            seen_categories = []  # порядок категорий как в файле
            category_items = {}   # category_name -> list of (item_text, price)
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    category_items[category_name].append((item_text, price))
            ...
            async with get_async_session_factory()() as session:
                ...
                for idx, category_name in enumerate(seen_categories):
                    category = await session.scalar(
                        select(Category).where(Category.name.ilike(category_name))
                    )

                    if not category:
                        # Новой категории даём sort_order по порядку в файле
                        new_order = max_order + (idx + 1) * order_step
                        category = Category(name=category_name, sort_order=new_order)
                        session.add(category)
                        await session.flush()
                        added_categories += 1

                    # Добавляем товары
                    for item_text, price in category_items[category_name]:
                        existing_item = await session.scalar(
                            select(Item).where(
                                Item.category_id == category.id,
                                Item.text.ilike(item_text)
    ...
    async def add_category(name: str) -> bool:
        ...
        async with get_async_session_factory()() as session:
            ...
            if existing:
                return False
            max_order = await session.scalar(select(func.max(Category.sort_order))) or 0
            category = Category(name=name, sort_order=max_order + 10)
            session.add(category)
            await session.commit()
            return True
