...
def is_likely_item(line: str) -> bool:
    ...
    Возвращает True, если:
      - в строке есть серийный номер в скобках;
      - строка начинается с "Б/У" или "NS" (маркеры категорий);
      - строка содержит скобки и хотя бы одну цифру (для аксессуаров с описанием).
    """
    stripped = line.strip()
    # Если есть серийный номер
    if extract_serials(line):
        return True
    # Если начинается с Б/У или NS
    if stripped.startswith('Б/У') or stripped.startswith('NS'):
        return True
    # Если есть скобки и цифры (например, "Чехол для iPhone 13 (черный)")
