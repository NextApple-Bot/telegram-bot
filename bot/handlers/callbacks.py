...
async def process_reset_finances(callback: CallbackQuery):
    ...
    try:
        if action == "confirm":
            ...
            await FinanceRepository.reset_today()
...
# -------------------------------------------------------------
# ОСНОВНОЙ ОБРАБОТЧИК МЕНЮ (теперь все функции определены выше)
# -------------------------------------------------------------
...
async def process_menu_callback(callback: CallbackQuery, bot, state):
    ...
    if action == "inventory":
        ...
        await process_remains(callback)   # теперь функция определена выше
