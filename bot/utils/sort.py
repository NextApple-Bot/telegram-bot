...
def extract_watch_size(text: str) -> int:
...
def get_full_model_name(item: str) -> str:
    without_brackets = re.sub(r'\([^)]*\)', '', item)
    return normalize_name(without_brackets)

def extract_base_name(item: str) -> str:
    without_brackets = re.sub(r'\([^)]*\)', '', item)
...
def parse_categories(lines: List[str]) -> List[Dict]:
    categories = []
    current_header = None
    current_items = []
    i = 0
...
def _add_category(categories: List[Dict], header: str, items: List[str]):
...
def sort_assortment_to_categories(input_text: str) -> List[Dict]:
    lines = input_text.splitlines()
    return parse_categories(lines)

def sort_items_in_category(items: List[str], header: str) -> List[str]:
...
def build_output_text(categories: List[Dict]) -> str:
    output_lines = []
    for cat in categories:
        header = cat['header']
...
def find_category_for_item(item: str, categories: List[Dict]) -> int:
...
def add_item_to_categories(item: str, categories: List[Dict]) -> tuple:
