...
# Предкомпилируем часто используемые регулярки
        ...
        for match in NUMBER_PATTERN.finditer(line):
            ...
            if open_paren != -1:
                ...
                if close_paren != -1 and close_paren > open_paren:
                    ...
                    found_keyword = False
                    for kw_re in PAYMENT_KEYWORDS.values():
                        if kw_re.search(bracket_content):
                            found_keyword = True
                            break
            ...
            for pay_type, kw_re in PAYMENT_KEYWORDS.items():
                if kw_re.search(line):
                    ...
                    logger.info(f"➕ {pay_type} += {amount} (строка: {line[:80]})")
                    break
