...
try:
    ...
    from bot.db import close_pool, get_pool   # ← ИМПОРТ ОБЯЗАТЕЛЕН
...
async def on_shutdown():
    ...
    try:
        await close_pool()   # ← ТЕПЕРЬ ОПРЕДЕЛЁН
    except Exception as e:
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=PORT, log_level="info")    if bot and dp:
        logger.info("✅ Бот и диспетчер готовы")
        if config and hasattr(config, 'RENDER_URL') and config.RENDER_URL:
            webhook_url = f"{config.RENDER_URL}/webhook"
            try:
