...
async def process_menu_callback(callback: CallbackQuery, bot, state):
    ...
    elif action == "finance":  # если нужна отдельная кнопка "Финансы", но мы объединили со статистикой
        # Если вы хотите отдельную кнопку, раскомментируйте этот блок
        s = stats.get_stats()
        text = (
            f"💰 Финансы за {s['date']}:\n"
            f"Терминал: {s['sales_terminal'] + s['preorders_terminal']:.0f} руб.\n"
...
# ... остальные callback-обработчики (confirm_clear, reset_stats, upload_mode и т.д.) остаются без изменений.
# Для полноты приведу их вкратце (они уже были в предыдущих версиях).
...
async def process_reset_stats(callback: CallbackQuery):
    ...
    if action == "confirm":
        ...
        stats.reset_stats()
        s = stats.get_stats()
        text = f"📊 Статистика за {s['date']} сброшена."
        await callback.message.edit_text(text)
    elif action == "no":
        s = stats.get_stats()
        text = f"📊 Статистика за {s['date']}:\n..."  # можно показать актуальную
        await callback.message.edit_text(text)
