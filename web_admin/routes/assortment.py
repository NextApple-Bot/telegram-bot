...
async def delete_item_and_log_sale(
    ...
    """Удаляет товар, сохраняет в deleted_items, добавляет статистику продаж и финансы с уникальным message_id."""
    pool = await get_pool()
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            # Добавляем статистику продажи с уникальным message_id
...
async def edit_item_submit(
    ...
    async with pool.acquire() as conn:
        ...
        if is_sold:
            ...
            # Генерируем уникальный message_id для продажи
